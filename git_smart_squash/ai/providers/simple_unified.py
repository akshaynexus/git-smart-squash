...
class UnifiedAIProvider:
    ...
    def _get_api_key(self, default_env: str) -> str:
        ...
        if self.config.ai.api_key_env:
            api_key = os.getenv(self.config.ai.api_key_env)
            if api_key:
                return api_key
    ...
    def _calculate_dynamic_params(self, prompt: str) -> dict:
        ...
        if prompt_tokens > provider_limit - 2000:  # Reserve 2000 for response
            raise Exception(
                f"Diff is too large ({prompt_tokens} tokens). Maximum supported: {provider_limit - 2000} tokens. Consider breaking down your changes into smaller commits."
            )
        ...
        response_buffer = max(
            2000, (prompt_tokens // 3) + 2000
        )  # Scale buffer with prompt size
    ...
    def _calculate_ollama_params(self, prompt: str) -> dict:
        ...
        if params["max_tokens"] > provider_limit * 0.8:
            ...
            min_context_needed = (
                int(estimated_prompt_tokens * 1.15) + 1000
            )  # 15% safety margin + response space
            num_ctx = max(params["max_tokens"], min_context_needed)
            # Respect absolute maximum
            num_ctx = min(num_ctx, provider_limit)

        return {"num_ctx": num_ctx, "num_predict": params["response_tokens"]}

    def generate(self, prompt: str) -> str:
        """Generate response using the configured AI provider."""
        handler = self._providers.get(self.provider_type)
        if not handler:
    ...
    def _generate_local(self, prompt: str) -> str:
        ...
        try:
            ...
            if supports_reasoning and self.config.ai.reasoning:
                ...
                logger.debug(
                    f"Using gpt-oss model with native reasoning level: {reasoning_level}"
                )
            else:
                full_prompt = prompt
                ...
                if self.config.ai.reasoning and not supports_reasoning:
                    logger.warning(
                        f"Model {self.config.ai.model} does not support native reasoning. "
            ...
            if result.returncode != 0:
                if result.returncode == 7:
                    raise Exception(
                        "Ollama request failed: Could not connect to Ollama server at localhost:11434. Please ensure Ollama is running."
                    )
                else:
            ...
            if response.get("done", True) is False:
                logger.warning(
                    f"Response may have been truncated. Used {ollama_params['num_ctx']} context tokens."
                )
            ...
            try:
                ...
                if (
                    ...
                    and "commits" in parsed
                    and isinstance(parsed["commits"], list)
                ):
            ...
            raise Exception(
                f"Ollama request timed out after {timeout} seconds. Try reducing diff size or using a faster model."
            )
    ...
    def _generate_openai(self, prompt: str) -> str:
        ...
        try:
            ...
            if (
                params["prompt_tokens"] > model_context_limit - 1000
            ):  # Reserve 1000 for response
                raise Exception(
            ...
            response_params = {
                ...
                "max_output_tokens": params.get(
                    "response_tokens", self.config.ai.max_predict_tokens
                ),
    ...
    def _generate_openrouter(self, prompt: str) -> str:
        ...
        try:
            ...
            raise Exception(
                ...
                f"Get your API key from https://openrouter.ai/settings"
            )
        except openai.APIStatusError as e:
            if e.status_code == 401:
                raise Exception(
                    f"OpenRouter authentication failed (401): User not found. "
                    f"Please verify your OPENROUTER_API_KEY is correct and your account is active. "
                    f"Get your API key from https://openrouter.ai/settings"
                )
            raise Exception(f"OpenRouter API error (status {e.status_code}): {e}")
        except Exception as e:
    ...
    def _generate_anthropic(self, prompt: str) -> str:
        ...
        try:
            ...
            model_context_limit = self.PROVIDER_MAX_CONTEXT_TOKENS.get(
                "anthropic", 200000
            )

            # Check if prompt exceeds model context limit
            if (
                params["prompt_tokens"] > model_context_limit - 4000
            ):  # Reserve 4000 for response
                raise Exception(
                    f"Prompt ({params['prompt_tokens']} tokens) exceeds {self.config.ai.model} context limit ({model_context_limit}). Consider reducing diff size."
                )
            ...
            if self.config.ai.reasoning:
                ...
                logger.debug(
                    f"Using Claude extended thinking with budget_tokens={budget_tokens} for reasoning={self.config.ai.reasoning}"
                )
        ...
        try:
            ...
            if self.config.ai.reasoning and supports_thinking:
                ...
                if thinking_budget == 0 and supports_thinking:
                    ...
                    logger.debug(
                        f"Model {self.config.ai.model} requires thinking, using low budget instead of disabling"
                    )
                logger.debug(
                    f"Using Gemini thinking budget={thinking_budget} for reasoning={self.config.ai.reasoning}"
                )
            ...
            response = client.models.generate_content(
                model=self.config.ai.model, contents=prompt, config=config
            )
            ...
            raise Exception(
                "Google Generative AI library not installed. Run: pip install google-genai"
            )
        except Exception as e:
